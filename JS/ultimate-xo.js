document.getElementById("ultimate-xo").onclick = function () {
  document.getElementById("hello").textContent =
    "Welcome In Eazy Ultimate Tic-Tac-Toe";
  ...
  new Ultimate_XO(9, 9, "Ultimate", 0, 0);
  window.scrollTo({
    top: 0,
    behavior: "smooth",
  });
...
class Ultimate_XO extends Board {
  constructor(x, y, gn, p1, p2) {
    ...
    document.getElementById("new-game").onclick = () => {
      new Ultimate_XO(9, 9, "Ultimate", p1, p2);
    };
    // Choosing PvP
      ...
      for (let j = 0; j < 3; j++) {
        if (
          ...
          if (this.almostWin(sym, idxx[i], idxy[j])) {
            ...
            for (let k = 0; k < 3; k++) {
              for (let f = 0; f < 3; f++) {
                collection[idxy[j] + idxx[i] * 9 + f + k * 9].className = "";
              }
            }
