document.getElementById("memory-xo").onclick = function () {
  document.getElementById("hello").textContent =
    "Welcome In Memory Tic-Tac-Toe";
...
class Memory_XO extends Board {
  constructor(x, y, gn, p1, p2) {
      ...
      ele.onclick = () => {
        ...
        if (ele.classList.contains("active")) {
          ...
          if (document.getElementById("computer").checked && !this.gameOver) {
            ...
            collection[y1 + x1 * y].innerHTML = "#";
            collection[y1 + x1 * y].style.color = "orange";
    ...
    document.getElementById("new-game").onclick = () => {
      new Memory_XO(3, 3, "Memory", p1, p2);
    };
    // Choosing PvP
    ...
    rulesList.innerHTML = `
            <li>It's a Memory Tic-Tac-Toe game</li>
            <li>Board size: 3x3</li>
            <li>The same rules as Normal Tic-Tac-Toe</li>
            <li>But you have to remember where you was 😑</li>
            <li>The first player to align 3 of their tokens in a row, column, or diagonal wins</li>
  ...
  stopAll() {
    ...
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 3; j++) {
        if (this.checkingBoard[i][j] != 0) {
          collection[j + i * 3].innerHTML =
            this.checkingBoard[i][j].toUpperCase();
          if (this.checkingBoard[i][j] == "x") {
            collection[j + i * 3].style.color = "#e74c3c";
          } else {
            collection[j + i * 3].style.color = "#3742fa";
          }
        }
      }
    }
